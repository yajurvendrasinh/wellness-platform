...
import { Menu, X, ArrowRight, ChevronDown, Copy, Check } from 'lucide-react';
  ...
  return (
    ...
    >
      {copied ? (
        ...
        <Copy size={14} className="text-on-surface-variant group-hover/copy:text-primary transition-colors" />
      )}
      {copied && (
        <motion.span 
    ...
    <nav className="fixed top-0 w-full z-50 glass-panel shadow-[0_20px_40px_rgba(41,24,2,0.06)]">
      <div className="flex justify-between items-center w-full px-8 py-6 max-w-7xl mx-auto">
        <div 
          ...
          className="text-xl font-bold tracking-tight text-on-surface font-alata cursor-pointer active:opacity-70"
        >
          Wellness
        </div>
        ...
        <div className="hidden md:flex gap-8 items-center">
          {[
            ...
            <button
              ...
              className={`font-alata tracking-tight transition-colors duration-300 cursor-pointer active:opacity-70 ${
                activeTab === item.id 
        ...
        <div className="hidden md:block">
          {/* Empty space - Sign In removed */}
        </div>

        <div className="md:hidden">
          <button onClick={() => setIsOpen(!isOpen)} className="text-primary-container">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="md:hidden bg-surface border-t border-outline-variant/10 px-8 py-4 space-y-4"
          >
            {['Calculators', 'Workout'].map((item) => (
              <button
                key={item}
                className="block w-full text-left font-alata text-on-surface-variant py-2"
                onClick={() => {
                  setIsOpen(false);
                  if (item === 'Calculators') setActiveTab('calculators');
                  if (item === 'Workout') setActiveTab('workout');
                }}
              >
                {item}
              </button>
...
const Digit: React.FC<{ value: string }> = ({ value }) => {
  ...
  return (
    <div className="inline-block overflow-hidden relative" style={{ height: '1em', lineHeight: '1em' }}>
      <div 
        ...
        style={{ transform: isNumeric ? `translateY(-${num * 10}%)` : 'translateY(0)' }}
      >
        {isNumeric ? (
          ...
          <span className="flex items-center justify-center" style={{ height: '1em' }}>
            {value}
          </span>
        )}
      </div>
  ...
  return (
    <div className="lg:col-span-7 flex flex-col justify-center">
      <motion.div 
        ...
        className="mb-12"
      >
        <h2 className="font-alata text-sm tracking-widest uppercase text-on-surface-variant mb-6">Your Daily Caloric Target</h2>
        <div className="flex items-end gap-3 mb-2">
          <div className="font-alata text-7xl md:text-8xl tracking-tighter text-primary flex items-baseline overflow-hidden h-[1em]">
            <Odometer value={adjustedData ? adjustedData.targetCalories.toLocaleString() : '-'} />
          </div>
      ...
      <div className="grid grid-cols-2 gap-8 mb-12 border-t border-outline-variant/15 pt-8">
        <div>
          <span className="block font-alata text-xs tracking-widest uppercase text-on-surface-variant mb-2">Basal Metabolic Rate</span>
          <span className="font-alata text-3xl text-secondary">
            {adjustedData ? adjustedData.bmr.toLocaleString() : '-'} <span className="text-sm text-on-surface-variant">kcal</span>
          </span>
        </div>
        <div>
          <span className="block font-alata text-xs tracking-widest uppercase text-on-surface-variant mb-2">Activity Energy</span>
          <span className="font-alata text-3xl text-secondary">
            {adjustedData ? adjustedData.activityEnergy.toLocaleString() : '-'} <span className="text-sm text-on-surface-variant">kcal</span>
          </span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-8 shadow-[0_20px_40px_rgba(41,24,2,0.03)] border border-outline-variant/10">
        <div className="mb-8">
          <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant mb-4">Select Caloric Goal</label>
        ...
        <h3 className="font-alata text-sm tracking-widest uppercase text-on-surface-variant mb-6">Macronutrient Baseline Focus</h3>
        <div className="space-y-6">
          <MacroBar 
            label="Protein" 
            grams={adjustedData?.macros.protein ?? 0} 
            calories={(adjustedData?.macros.protein ?? 0) * 4} 
            percentage={adjustedData && results ? Math.round(((adjustedData.macros.protein * 4) / results.tdee) * 100) : 0} 
            color="bg-primary" 
          />
          <MacroBar 
            label="Fats" 
            ...
            percentage={adjustedData && results ? Math.round(((adjustedData.macros.fats * 9) / results.tdee) * 100) : 0} 
            color="bg-secondary" 
          />
          <MacroBar 
            label="Carbohydrates" 
            ...
            percentage={adjustedData && results ? Math.round(((adjustedData.macros.carbs * 4) / results.tdee) * 100) : 0} 
            color="bg-tertiary" 
          />
        </div>
      </div>
...
const MacroBar = ({ label, grams, calories, percentage, color }: { label: string, grams: number, calories: number, percentage: number, color: string }) => (
  <div>
    <div className="flex justify-between mb-2">
      <span className="font-medium text-on-surface">{label}</span>
      <span className="text-on-surface-variant">{grams}g / {calories.toLocaleString()} kcal</span>
    </div>
...
const TDEECalculator = () => {
  ...
  const isFormValid = useMemo(() => {
    ...
    if (inputs.unit === 'imperial') {
      heightValid = parseFloat(inputs.heightFt) > 0 || parseFloat(inputs.heightIn) > 0;
    } else {
      ...
      <div className="lg:col-span-5 bg-surface-container-low rounded-xl p-8 md:p-10">
        <form className="space-y-8" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="space-y-3">
            <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant">Biological Sex</label>
          ...
          {/* Age */}
          <div className="space-y-3">
            <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant" htmlFor="age">Age</label>
            <input 
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-3">
              <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant">Height</label>
              {inputs.unit === 'imperial' ? (
                <div className="flex gap-2">
                  <input className="w-full bg-surface-container-high border-none rounded-xl px-4 py-4 text-on-surface focus:ring-2 focus:ring-outline-variant/30 focus:bg-primary-fixed outline-none" placeholder="Ft" type="number" value={inputs.heightFt} onChange={(e) => handleChange('heightFt', e.target.value)} />
                  <input className="w-full bg-surface-container-high border-none rounded-xl px-4 py-4 text-on-surface focus:ring-2 focus:ring-outline-variant/30 focus:bg-primary-fixed outline-none" placeholder="In" type="number" value={inputs.heightIn} onChange={(e) => handleChange('heightIn', e.target.value)} />
                </div>
              ) : (
                <input className="w-full bg-surface-container-high border-none rounded-xl px-4 py-4 text-on-surface focus:ring-2 focus:ring-outline-variant/30 focus:bg-primary-fixed outline-none" placeholder="cm" type="number" value={inputs.heightCm} onChange={(e) => handleChange('heightCm', e.target.value)} />
              )}
            </div>
            <div className="space-y-3">
              <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant">Weight</label>
              <div className="relative">
                <input className="w-full bg-surface-container-high border-none rounded-xl px-5 py-4 text-on-surface focus:ring-2 focus:ring-outline-variant/30 focus:bg-primary-fixed outline-none pr-12" placeholder="0" type="number" value={inputs.weight} onChange={(e) => handleChange('weight', e.target.value)} />
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">{inputs.unit === 'imperial' ? 'lbs' : 'kg'}</span>
              </div>
            </div>
          </div>

          <div className="space-y-3">
            <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant" htmlFor="activity">Activity Level</label>
            <div className="relative">
              <select 
                className="w-full select-wellness pr-12" 
                id="activity"
                value={inputs.activity}
                onChange={(e) => handleChange('activity', e.target.value as ActivityLevel)}
              >
                {Object.entries(ACTIVITY_LABELS).map(([val, label]) => (
                  <option key={val} value={val} className="bg-surface py-2 text-on-surface">{label}</option>
                ))}
              </select>
          ...
          <div className="space-y-3 pt-4 border-t border-outline-variant/15">
            <label className="block font-alata text-sm tracking-widest uppercase text-on-surface-variant flex justify-between">
              <span>Body Fat %</span>
              <span className="text-xs text-on-surface-variant/70 normal-case tracking-normal">Optional for accuracy</span>
            </label>
            <div className="relative">
              <input 
  ...
  return (
    <div className="max-w-5xl mx-auto py-12">
      <header className="mb-16 text-center max-w-3xl mx-auto">
        <h1 className="font-alata text-4xl md:text-5xl text-primary-container tracking-tight mb-6 mt-12 text-center">Curate Your Intent</h1>
        <p className="text-on-surface-variant text-lg leading-relaxed text-center">
          Select your focus and dedicated time. We will architect a bespoke routine grounded in a mindful wellness ethos.
        </p>
      </header>
        ...
        {showRoutine && (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <h3 className="font-alata text-2xl text-primary-container tracking-tight px-4 whitespace-nowrap">Your Curated Routine</h3>
              <div className="h-px bg-outline-variant/30 flex-grow" />
            </div>
            ...
            <div className="bg-white rounded-xl shadow-[0_20px_40px_rgba(41,24,2,0.04)] overflow-hidden border border-outline-variant/10 text-left">
              <div className="px-8 py-5 bg-surface-container-low border-b border-outline-variant/15 flex justify-between items-center">
                <span className="font-alata text-primary-container font-bold text-sm tracking-widest uppercase">
                  {currentWorkout?.type === 'strength' ? `Theme: ${currentWorkout?.theme}` : `Focus: ${currentWorkout?.title}`}
                </span>
              ...
              {currentWorkout?.type === 'strength' ? (
                <>
                  ...
                  <div className="divide-y divide-outline-variant/10">
                    {currentWorkout?.exercises?.map((m: any, i: number) => (
                      <div key={i} className="grid grid-cols-12 gap-4 px-8 py-6 items-center hover:bg-surface-container-low/50 transition-colors group">
                        <div className="col-span-6">
                          <div className="flex items-center flex-wrap">
                            <span className="font-alata text-lg text-primary">{m.name}</span>
                            <CopyButton text={m.name} />
                          </div>
                          <span className="text-sm text-on-surface-variant font-medium block">{m.coaching_cue || m.desc}</span>
                        </div>
                        ...
                        <div className="col-span-3 text-right">
                          <span className="block font-semibold">{m.sets} Sets</span>
                          <span className="text-xs text-on-surface-variant font-medium">{m.rep_range} Reps</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <>
                  <div className="grid grid-cols-12 gap-4 px-8 py-5 bg-surface-container-low/50 border-b border-outline-variant/15 font-bold text-xs tracking-wider uppercase">
                    ...
                    <div className="col-span-5 text-right">Metric / Duration</div>
                  </div>
                    ...
                    {currentWorkout?.structure?.map((phase: any, i: number) => (
                      <div key={i} className="grid grid-cols-12 gap-4 px-8 py-6 items-center hover:bg-surface-container-low/50 transition-colors group">
                        <div className="col-span-7">
                          <div className="flex items-center flex-wrap">
                            <span className="font-alata text-lg text-primary">{phase.phase}</span>
                            <CopyButton text={phase.phase} />
                          </div>
                          <span className="text-sm text-on-surface-variant font-medium block">{phase.description}</span>
                        </div>
                        <div className="col-span-5 text-right">
                          <div className="flex flex-col items-end gap-1">
                            <span className="block font-semibold">{phase.duration_min} min</span>
                            <span className="inline-block px-3 py-1 rounded-full text-[10px] font-bold bg-surface-container-high text-on-surface uppercase tracking-wider">
                  ...
                  {currentWorkout?.rp_principle && (
                    <div className="px-8 py-6 bg-surface-container-low/30 border-t border-outline-variant/15 italic text-sm text-on-surface-variant">
                      <span className="font-bold uppercase tracking-widest text-[10px] block mb-1 not-italic opacity-70">Scientific Insight</span>
                      {currentWorkout.rp_principle}
                    </div>
                  )}
                </>
                ...
                <>
                  ...
                  <div className="divide-y divide-outline-variant/10">
                    {currentWorkout?.drills?.map((drill: any, i: number) => (
                      <div key={i} className="grid grid-cols-12 gap-4 px-8 py-6 items-start hover:bg-surface-container-low/50 transition-colors group">
                        <div className="col-span-8">
                          <div className="flex items-center flex-wrap">
                            <span className="font-alata text-lg text-primary leading-tight mb-1">{drill.name}</span>
                            <CopyButton text={drill.name} />
                          </div>
                          <span className="text-sm text-on-surface-variant leading-relaxed block">{drill.instruction}</span>
                        </div>
              ...
              <div className="px-8 py-4 bg-surface-container-low/30 border-t border-outline-variant/15 flex justify-between items-center text-xs opacity-60">
                <span>Modality: {currentWorkout?.modality || 'Gym/Bodyweight'}</span>
...
const SelectionGroup = ({ title, options, value, onChange }: { title: string, options: { id: string, sub: string }[], value: string | null, onChange: (v: string) => void }) => (
  <div className="flex flex-col space-y-6 text-left">
    <h2 className="font-bold text-lg text-on-surface tracking-wide uppercase border-b border-outline-variant/30 pb-3">{title}</h2>
      ...
      <main className="flex-grow w-full max-w-7xl mx-auto px-6 py-24 md:py-32">
        <AnimatePresence mode="wait">
          {activeTab === 'calculators' ? (
            ...
            >
              <header className="mb-16 max-w-2xl">
                ...
                <p className="text-on-surface-variant text-lg leading-relaxed">
                  Determine how many calories your body burns each day, establishing your baseline for a mindful approach to wellness and nutrition.
                </p>
              </header>
              <TDEECalculator />
            </motion.div>
            ...
            >
              <h2 className="text-3xl font-alata text-primary-container">Bespoke Wellness Awaits</h2>
              <p className="text-on-surface-variant mt-4">We are curating the finest content for you.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </main>
